...
public class Draw {
    ...
    public static void draw() {
        ...
        for (Line line : lines) {
            for (int i=0; i < 2; i++){
                Vector2f position;
                if (i == 0) position = line.getFrom();
                else position = line.getTo();
    ...
    public static void addLine2D(Vector2f from, Vector2f to, Vector3f color, int lifetime) {
        ...
        Draw.lines.add(new Line(from, to, color, lifetime));
    }
    ...
    public static void addBox2D(Vector2f center, Vector2f dimensions, float rotation, Vector3f color, int lifetime) {
        ...
        if (rotation != 0.0f) {
            for (Vector2f vert : vertices) {
                    GamesMath.rotate(vert, rotation, center);
            }
        }
    ...
    public static void addCircle(Vector2f center, float radius, Vector3f color, int lifetime) {
        ...
        for (int i = 0; i < points.length; i++) {
            ...
            GamesMath.rotate(tmp, currentAngle, new Vector2f());
